...
! defined( 'WOOSQ_VERSION' ) && define( 'WOOSQ_VERSION', '2.1.0' );
		...
		if ( ! class_exists( 'WPCleverWoosq' ) ) {
			class WPCleverWoosq {
				...
				function woosq_available_variation( $data, $variable, $variation ) {
					if ( $woosq_image_id = $variation->get_image_id() ) {
						$woosq_image_src         = wp_get_attachment_image_src( $woosq_image_id, get_option( 'woosq_image_size', 'woosq' ) );
						$data['woosq_image_id']  = $woosq_image_id;
						$data['woosq_image_src'] = $woosq_image_src[0];
						$data['woosq_image']     = wp_get_attachment_image( $woosq_image_id, get_option( 'woosq_image_size', 'woosq' ) );
					}
                                    ...
                                    <div class="thumbnails thumbnails-ori">
										...
										if ( ! empty( $thumb_ids ) ) {
											foreach ( $thumb_ids as $thumb_id ) {
												echo '<div class="thumbnail">' . wp_get_attachment_image( $thumb_id, get_option( 'woosq_image_size', 'woosq' ) ) . '</div>';
											}
										} else {
											echo '<div class="thumbnail">' . wc_placeholder_img( 'woosq' ) . '</div>';
										}
										?>
                                    </div>
							...
							<?php if ( $active_tab === 'settings' ) { ?>
                                <form method="post" action="options.php">
                                    ...
                                    <table class="form-table">
                                        ...
                                        <tr>
                                            <th scope="row"><?php esc_html_e( 'Image size', 'woosq' ); ?></th>
                                            <td>
												<?php
												...
												if ( ! empty( $image_sizes ) ) {
													echo '<select name="woosq_image_size">';
                                        ...
                                        <tr class="submit">
                                            <th colspan="2">
                                                ...
                                                <input type="hidden" name="page_options"
                                                       value="woosq_button_type,woosq_button_text,woosq_button_class,woosq_button_position,woosq_effect,_woosq_cats,woosq_content_image,woosq_image_size,woosq_content_image_lightbox,woosq_summary,woosq_add_to_cart_button,woosq_content_view_details_button,woosq_content_view_details_text"/>
                                            </th>
                                        </tr>
                                    </table>
                                </form>
